...
import HerokuLogo from "@/components/HerokuLogo";
...
export default function ChatBubble({ welcomeMessage, agentId, agentName }: ChatBubbleProps) {
  ...
  return (
    <>
      {!isOpen ? (
        ...
        <div className={`
          fixed z-50 inset-0
          ${!isExpanded ? 'md:inset-auto md:top-auto md:right-4 md:bottom-4 md:left-auto md:z-[100]' : 'md:z-[200]'}
        `}>
          ...
          <div className={`
            ...
            md:h-auto md:w-auto
            ${isExpanded 
              ? 'md:max-w-4xl md:max-h-[80vh] md:mx-auto md:my-auto md:rounded-xl md:relative' 
              : 'md:w-[480px] md:h-[700px] md:max-h-[calc(100vh-2rem)] md:rounded-xl'
            }
          `}>
            {/* Header */}
            <div className="flex justify-between items-center px-4 py-3 border-b bg-white/95 backdrop-blur-sm md:bg-white md:backdrop-blur-none md:rounded-t-xl">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="text-gray-900 truncate block text-xl font-medium">{displayName}</h1>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-hidden">
              <ChatContainer welcomeMessage={welcomeMessage} agentId={agentId} />
            </div>
          </div>
        </div>
      )}
